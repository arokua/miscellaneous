...
public class TicTacToe5x5 extends PApplet {
...
class Button {
  ...
  int c;    //color of the button
  int cs;   //color of the stroke for the button
  int t; // color of the text
  float sWeight; //stroke weight for the rect of the button
  float tSize; //text size
  
  // THe parameter of the button
  Button(String label2, float xpos, float ypos, float widthB, float heightB, int bC, int cS, int tC, float sW, float TS) {
    label = label2;
    x = xpos;
    y = ypos;
    w = widthB;
...
int CurrentScreenColor = 0;
int CurrentXColor = color(255, 0, 0);
int CurrentOColor = color(0, 0, 255);
boolean pvc = true;
int XWins = 0;
int OWins = 0;
float a1 = 0;
...
public void Setting() {
  ...
  int constrast = color(255 - red(CurrentScreenColor), 255 - green(CurrentScreenColor), 255 - blue(CurrentScreenColor));
...
public void draw() {
  if (gameState == "START") {
    ...
    int contrast = color(255 - red(CurrentScreenColor), 255 - green(CurrentScreenColor), 255 - blue(CurrentScreenColor));
    ...
    drawStar(random(mouseX - 5, mouseX + 5), random(mouseY -5, mouseY + 5), PApplet.parseInt(random(10, 25)), random(0, 255));
    squareHighlight();    
  }
}
      ...
      for (int i = 0; i < 5; i++) {
        ...
        for (int j = 0; j < 5; j++) {
          if ( gridSide * i < mouseX && gridSide * (i+1) > mouseX &&
               ...
               (chosen.hasValue(PApplet.parseInt(str(i)+str(j))) == false)) {
                 
             if (check == 0){
               chosen.append(PApplet.parseInt(str(i)+str(j)));
               drawX(i * gridSide, j * gridSide + 30, CurrentXColor);
               check += 1;
               Xchosen.append(PApplet.parseInt(str(i)+str(j)));
               
             }else if (check == 1 && pvc == false){
               chosen.append(PApplet.parseInt(str(i)+str(j)));
               drawO(i * gridSide, j * gridSide + 30);
               check -= 1;
               Ochosen.append(PApplet.parseInt(str(i)+str(j)));
    ...
    for (int i = 0; i < 5; i++) {
      for (int j = 0; j < 5; j++) {
       ...
       if ( gridSide * i < mouseX && gridSide*(i+1) > mouseX && 
        ...
        }else if (chosen.hasValue(PApplet.parseInt(str(i)+str(j))) == false) {
          noFill();
          rect(i * gridSide, j * gridSide + 30, gridSide, gridSide);  
        }else if (Xchosen.hasValue(PApplet.parseInt(str(i)+str(j))) == true ) {
           drawX(i * gridSide, j * gridSide + 30, CurrentXColor);
        }else if (Ochosen.hasValue(PApplet.parseInt(str(i)+str(j))) == true) {
          drawO(i * gridSide, j * gridSide + 30);
        }
      }
    }
...
public void drawStar(float x, float y, int s, float o) {
...
public void winScreen(String winner){  
 ...
 textSize(30);
 text("X: " + XWins + " win(s)", 0.2f * width, 0.2f *height);
 text("O: " + OWins + " win(s)", 0.8f * width, 0.2f *height);
 
 restart = new Button("Replay", 0.5f * width -30, 200, 80, 30, CurrentScreenColor, color(0xff1C459B), 255 , 3, 20);
 restart.Draw();
...
public boolean checkWin(IntList moveList){
...
public void drawBlankBoard(int r, int c) {
...
public void drawX( float x, float y, int X_Color){
  noFill();
...
public void MovingBuilding() {
  ...
  for (int h =0; h <10; h++){
    int floor=PApplet.parseInt(random(4, 20));
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "TicTacToe5x5" };
    if (passedArgs != null) {
